...
export default async function handler(req, res) {
  ...
  if (req.method === 'POST') {
    try {
      ...
      const { name, score, ts } = body || {};
      if (!name || !score) return res.status(400).json({ error: 'Missing name or score' });
      const scores = await redis.get(key) || [];
      scores.push({ name: String(name).substring(0, 20).toUpperCase(), score: parseInt(score), ts: ts || Date.now() });
      scores.sort((a, b) => b.score - a.score);
      const top25 = scores.slice(0, 25);
      await redis.set(key, top25, { ex: 60 * 60 * 24 * 14 });
